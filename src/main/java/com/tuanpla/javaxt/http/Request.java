...
public class Request {
  ...
  //**************************************************************************
     ...
     * encoded using "Basic" authentication. Note that credentials encoded using
     * "Basic" authentication can be easily decoded. As a general rule, do not
     * pass credentials to sites that are not secured using SSL.
     */
  ...
  //**************************************************************************
    ...
    private String getCredentials() throws Exception {
        if (username == null || password == null) {
            ...
            return com.tuanpla.utils.Base64.encodeBytes(
  ...
  //**************************************************************************
    ...
    //**************************************************************************
    /**
     * Used to post an array of form inputs to a server. Form inputs can include
     * text or binary data, including files. Payload is normally
     * "multipart/form-data" encoded.
     */
    public void write(com.tuanpla.javaxt.html.Input[] inputs) {

        //Generate boundary
        String boundary = "---------------------------";
        for (int i = 0; i < 14; i++) {
            boundary += new java.util.Random().nextInt(10);
        }
        ...
        try {
            ...
            for (int i = 0; i < inputs.length; i++) {
                ...
                com.tuanpla.javaxt.html.Input input = inputs[i];
                String contentType = "";
                String fileName = "";
                long inputLength = 0;

                if (input.isFile()) {
                    com.tuanpla.javaxt.io.File file = (com.tuanpla.javaxt.io.File) input.getValue();
            ...
            try {
                ...
                for (int i = 0; i < inputs.length; i++) {
                    ...
                    //Write input value
                    if (inputs[i].isFile()) {
                        com.tuanpla.javaxt.io.File file = (com.tuanpla.javaxt.io.File) inputs[i].getValue();
    ...
    private URLConnection connect(boolean doOutput) {
        try {
          ...
          //By default, Java 1.6 and earlier use the SSLv2 protocol to initiate
            ...
            url = new com.tuanpla.utils.URL(url).toURL();
        ...
        if (conn != null) {
            ...
            if ((responseCode >= 300 && responseCode < 400) && maxRedirects > 0) {
                ...
                while (responseCode >= 300 && responseCode < 400) {
                    ...
                    if (useCache && responseCode == 304) {
                        break;
                    }

                    //Parse location response header
                    String location = getResponseHeader("Location");
                    com.tuanpla.utils.URL newUrl = new com.tuanpla.utils.URL(location);
                    if (newUrl.getProtocol() == null) {
                        com.tuanpla.utils.URL url = new com.tuanpla.utils.URL(this.url);
  ...
  //**************************************************************************
    ...
    //**************************************************************************
    /**
     * Used to parse the first line from the http response. Note that once, this
     * method is called, clients can no longer write to the socket!
     */
    private void parseResponse(URLConnection conn) {
        ...
        if (!headers.isEmpty()) {
            ...
            if (status != null) {
                ...
                if (st.hasMoreTokens()) {
                    protocol = st.nextToken().trim().toUpperCase();
                }
                ...
                if (st.hasMoreTokens()) {
                    responseCode = com.tuanpla.utils.string.toInt(st.nextToken().trim());
                }
                if (st.hasMoreTokens()) {
                    message = "";
  ...
  //**************************************************************************
    ...
    protected String getResponseHeader(String headerName) {
        ...
        if (arr.length > 0) {
            return getResponseHeaders(headerName)[0];
        }
        return null;
    }

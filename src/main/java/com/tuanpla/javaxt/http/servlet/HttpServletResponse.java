...
import com.tuanpla.javaxt.http.Server.SocketConnection;
...
public class HttpServletResponse {
            ...
            for (String kvp : range.split(";")){ //can there be multiple keys?
                ...
                if (eq>0){
                    ...
                    if (key.equals("bytes")){
                        ...
                        try{ startRange = Long.parseLong(arr[0].trim()); }catch(Exception e){}
                        try{ endRange = Long.parseLong(arr[1].trim()); }catch(Exception e){}
                    }
                }
            }
        }
  ...
  //** setContentLength
  //**************************************************************************
  /** Sets the "Content-Length" in the response header.
   */
   ...
   */
    public void write(java.io.InputStream inputStream, boolean compressOutput) throws IOException {
      ...
      //Write body. Compress as needed.
        ...
        if (gzip) out = new GZIPOutputStream(new ConnectionOutputStream(), bufferSize);
        else out = new ConnectionOutputStream();
        byte[] b = new byte[bufferSize];
        int x=0;
        while ( (x = inputStream.read(b)) != -1) {
            out.write(b,0,x);
   ...
   */
    private void writeChunk(ByteBuffer buf) throws IOException {
      ...
      //Late check to see if we should chunk the output
        ...
        if (buf.position()==buf.capacity()){ //Send all the bytes to the client
        ...
        else { //Buffer is only partially filled. This is our queue that we're
          ...
          //Trim the byte buffer
            ...
            buf.rewind();
            buf.get(arr);
            buf = buf.allocateDirect(arr.length);
            buf.put(arr);
            arr = null;
      ...
      //buffer as needed.
        if (startRange!=null && !writeHeader){
            ...
            if ((ttl+length)<startRange){
              ...
              //contain bytes we need to satisfy the request. Resize the buffer
              //accordingly.
            ...
            if (endRange!=null){
                if (ttl>endRange){
                    ...
                    if (remainingBytes<length){
                        ...
                        buf.rewind();
                        buf.get(arr);
                        buf = buf.allocateDirect(length);
                        buf.put(arr);
                        buf.flip();
                    }
                    else{
                        //System.err.println("Send bytes? Range (" + startRange + "-" + endRange + "). "
                        //        + "Written " + (ttl) + "bytes. Need to send " + remainingBytes + " more");
                    }
                }
            }
            else{ //no end range specified and content length is unknown
                
                //System.err.println("No end range specified and content length is unknown? "
                //        + "Range (" + startRange + "-" + endRange + "). Written " + (ttl) + " bytes. Buffer is " + length);
            }
        }
  ...
  //** getCRLF
  //**************************************************************************
  /**  Used to create and fill a ByteBuffer with a Carriage Return (CR) and
   *   Line Feed (LF).
   */
  ...
  //** getChannel
  //**************************************************************************
  /**  Returns the raw NIO SocketChannel associated with this response. Use
   *   with care.
   */
    ...
    public String getHeader(){
      ...
      //Add headers
        ...
        while (it.hasNext()){
            ...
            if (val!=null) header.append(key + ": " + val + "\r\n");
        }
      ...
      //Add cookies
        if (!cookies.isEmpty()){
            ...
            while (cookie.hasNext()){
                ...
                if (cookie.hasNext()) header.append(" ");
            }
            header.append("\r\n");
        }

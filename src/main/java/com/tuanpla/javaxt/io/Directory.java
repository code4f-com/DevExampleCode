...
public class Directory implements Comparable {
    ...
    public static Directory[] getRootDirectories(){
      ...
      //from the listRoots() method (e.g. disconnected mounted drives).
        if (isWindows){
            ...
            if (doNetUse){
                com.tuanpla.javaxt.io.Shell cmd = new com.tuanpla.javaxt.io.Shell("net use");
                cmd.run();
      ...
      //Special case for a directory whose path represents a server name (e.g. "\\192.168.0.1")
        if (isWindows && path.startsWith("\\\\")){
            ...
            if (!path.substring(2).contains(PathSeparator)){
                ...
                if (doNetUse){
                    com.tuanpla.javaxt.io.Shell cmd = new com.tuanpla.javaxt.io.Shell("net view " + path);
                    cmd.run();
   ...
   */
    public java.util.Date getDate(){
        return new com.tuanpla.utils.Date(Directory.lastModified()).getDate();
    }
    ...
    public File[] getFiles(Object filter){
        ...
        if (Directory.exists()){
            ...
            if (files==null){
                ...
                java.util.ArrayList<com.tuanpla.javaxt.io.File> list = new java.util.ArrayList<com.tuanpla.javaxt.io.File>();
                for (int i=0; i<files.length; i++){
                    ...
                    if (file.exists() && file.isFile()){
                        list.add(new com.tuanpla.javaxt.io.File(file));
                    }
                }
                Collections.sort(list, new FileComparer(this));
                return list.toArray(new com.tuanpla.javaxt.io.File[list.size()]);
            }
        }
        else{
            return new File[0];
        ...
        if (isWindows){
          ...
          //hidden network drives.
            if (doNetView){
                ...
                com.tuanpla.javaxt.io.Shell cmd = new com.tuanpla.javaxt.io.Shell("net view " + serverName);
                cmd.run();
                java.util.List errors = cmd.getErrors();
                errors.remove(null);
                if (errors.isEmpty()){
      ...
      //If we're still here, list files using a command prompt
        ...
        try{
          ...
          //Execute a windows shell command to get a directory listing
            com.tuanpla.javaxt.io.Shell cmd = new com.tuanpla.javaxt.io.Shell("cmd.exe /c dir /OG " + path);
            java.util.List<String> output = cmd.getOutput();
            cmd.run(true);
